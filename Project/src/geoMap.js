...
export function GeoMap(props) {
    const { offsetX, offsetY, map, data, height, width, selectedIncident, setSelectedIncident, mouseOver, mouseOut } = props;
    const projection = geoMercator().fitSize([width, height], map);
    const path = geoPath(projection);
    const r = scaleLinear().range([2, 20]).domain([min(data, d => d.casualty), max(data, d => d.casualty)]);
    ...
    const getColor = (selectedIncident, incident) => {
        return incident === selectedIncident ? "steelblue" : "red";
    }
    ...
    return <g transform={`translate(${offsetX}, ${offsetY})`}>
        ...
        {data.map(d => {
            ...
            return <circle key={d.incident_id + d.longitude + d.latitude}
                cx={x} cy={y} r={r(d.casualty)} opacity={0.7} stroke={"black"}
